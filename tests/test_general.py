...
from fragmap.generate_matrix import BriefFragmap, CellKind, Fragmap
...
def MockDiffDelta(old_file_path, new_file_path):
    return Mock(
        old_file=MockDiffFile(old_file_path),
...
class Test(unittest.TestCase):
    ...
    def test_011_012(self):
        self.check_diffs(
            ["011-add-x-to-A-and-N", "012-add-x-to-A-C"], ["#.#", "##."]
        )
    ...
    def test_020_021(self):
        self.check_diffs(
            ["020-modfile-create", "021-modfile-remove-first-line"],
            ["##", "#."],
        )

    def test_022_023(self):
        self.check_diffs(
            ["022-modfile-mod-second-line", "023-modfile-readd-first-line"],
            [".#", "#."],
        )
    ...
    def test_030_031(self):
        self.check_diffs(
            ["030-addmod-create-with-ab", "031-addmod-add-c"], ["#.", ".#"]
        )

    def test_030_032(self):
        self.check_diffs(
            [
                "030-addmod-create-with-ab",
                "031-addmod-add-c",
    ...
    def test_060_061(self):
        self.check_diffs(
            ["060-binaryfile-added", "061-binaryfile-changed"], ["#", "#"]
        )

    def test_070_072(self):
        self.check_diffs(
            ["070-add-X-to-D", "071-add-X-to-A", "072-add-X-to-F"],
            [".#.", "#..", "..#"],
        )
    ...
    def test_110_111_brief(self):
        self.check_diffs_brief(
            [
                "110-realdiff-newfilebug-addfile",
    ...
    def test_016_004_files(self):
        ...
        self.check_file_selection(
            {FileId(-1, "empty.txt")}, ["empty.txt"], start
        )
        self.check_file_selection(
            {FileId(-1, "empty.txt")}, ["other.txt"], start
        )
    ...
    def test_050_054_files(self):
        start = "0e427042"
        self.check_file_selection(
            {FileId(-1, "file_a.txt")}, ["file_a.txt"], start
        )
        self.check_file_selection(
            {FileId(0, "file_b.txt")}, ["file_b.txt"], start
        )
        self.check_file_selection(
            {FileId(0, "file_b.txt"), FileId(-1, "file_a.txt")}, None, start
        )

    # === Exception tests on more complex commits ===

    def test_1622573011(self):
        self.check_no_exceptions(
            [
                "Prepare for smarter propagation",
    ...
    def check_diffs(self, commit_messages, matrix):
        ...
        repo_path = os.path.join(DIFF_DIR, "build", test_name)
        commit_hexes = [
            find_commit_with_message(repo_path, commit_message)
    ...
    def check_file_selection(self, expected_ids, file_arg, start_commit):
        ...
        repo_path = os.path.join(DIFF_DIR, "build", test_name)
        cl = CommitLoader()
        all_commits_in_repo = CommitSelection(
            start_commit, None, 999, True, True
        )
