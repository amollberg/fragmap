...
from fragmap.load_commits import STAGED_HEX, UNSTAGED_HEX
...
def create_bundles(test_dir):
    ...
    test_subdirs = [
        subdir
        for subdir in subdirs(test_dir)
...
def unbundle_execution_test_repos(test_dir):
    test_bundles = [
        filename
        for filename in files(test_dir)
        if basename(filename).startswith("test_")
        and filename.endswith(".bundle")
    ]
...
def remove_execution_test_repos(test_dir):
    test_subdirs = [
        subdir
        for subdir in subdirs(os.path.join(test_dir, "build"))
        if basename(subdir).startswith("test_")
    ]
